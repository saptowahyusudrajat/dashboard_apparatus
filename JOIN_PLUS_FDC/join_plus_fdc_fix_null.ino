...
// Hold-last-value for thermocouples on transient fault (hindari NaN di Node-RED)
// Index 0..4 = sensor 1..5
float lastValidTemp[5] = {NAN, NAN, NAN, NAN, NAN};
...
// Baca thermocouple dengan "hold last value": jika fault/NAN, kembalikan nilai
// valid terakhir agar grafik di Node-RED tidak putus. idx = 0..4 (sensor 1..5).
float readTempC(Adafruit_MAX31855 &sensor, int idx) {
  float temp = sensor.readCelsius();
  if (isnan(temp) || (sensor.readError() != 0)) {
    return lastValidTemp[idx];   // pakai nilai sebelumnya
  }
  lastValidTemp[idx] = temp;     // simpan sebagai nilai valid terakhir
  return temp;
}
...
void loop() {
  ...
  if (!isnan(pressure)) {
    // Read all sensors
    float temp1 = readTempC(thermocouple1, 0);
    float temp2 = readTempC(thermocouple2, 1);
    float temp3 = readTempC(thermocouple3, 2);
    float temp4 = readTempC(thermocouple4, 3);
    float temp5 = readTempC(thermocouple5, 4);
